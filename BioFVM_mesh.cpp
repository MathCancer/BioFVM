...
int General_Mesh::voxel_face_index( int i, int j )
{
 
 // got == mymap.end()

 return voxel_face_index_mapping[i][j]; 
}
*/

/*
// only in a future release. 
Voxel_Face& General_Mesh::voxel_face( int i, int j )
{
 return voxel_faces[ voxel_face_index_mapping[i][j] ]; 
}
*/

/*
// only in a future release. 
std::vector<double>& General_Mesh::outward_normal( int i, int j )
{
	if( i < j )
	{ return voxel_faces[ voxel_face_index_mapping[i][j] ].outward_normal; }
	return voxel_faces[ voxel_face_index_mapping[i][j] ].inward_normal; 
}
*/
...
// only in a future release. 
	...
	// add it to the connectivity map 
	
	voxel_face_index_mapping[i][j] = k; 
	
	// add j to the list of connected voxels for voxel i 
	
//	connected_voxel_indices[i].push_back( j ); 
	
	voxel_face_index_mapping[j][i] = k; 
...
//	
//	Voxel_Face VF2 = VF1; 
//	VF2.outward_normal *= -1.0;
//	k++; 
//	VF2.mesh_index = k; 
//	
//	voxel_faces.push_back( VF2 ); 
//	voxel_face_index_mapping[j][i] = k; 
//	connected_voxel_indices[j].push_back( i ); 
*/
...
/*
// only in a future release. 
void General_Mesh::clear_voxel_face_index_mapping( void )
{ voxel_face_index_mapping.clear(); }
*/
...
void General_Mesh::read_from_matlab( std::string filename )
{
	...
	/*
	// only in a future release 
	clear_voxel_face_index_mapping();
	*/
...
/*
std::ostream& operator<<(std::ostream& os, const Cartesian_Mesh& mesh)  
{
	std::boolalpha( os ); 
	static std::string tabbing = "\t\t"; 
	static std::string tabbing2 = "\t\t\t"; 
	os	<< tabbing << "<mesh type=\"Cartesian\" uniform=\"" << mesh.uniform_mesh << "\" units=\"" << mesh.units << "\">"  << std::endl
		<< tabbing2 << "<voxels>" << std::endl;
	for( int i=0; i < mesh.voxels.size() ; i++ )
	{ os << mesh.voxels[i] << std::endl; } 
	os 	<< tabbing2 << "</voxels>" << std::endl 
